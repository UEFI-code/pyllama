...
def load(
    ...
    if os.path.exists('theHack/BadTransfomer'):
        os.chdir('theHack')
    else:
    ...
    os.chdir('BadTransfomer')
    model.layers = []
    for i in range(32):
        model.layers.append(torch.load('theBadTransformerLayer%d.pth' % i, map_location='cpu'))
...
def run(
    ...
    world_size = 1
    generator = load(
        tokenizer_path, local_rank, world_size, max_seq_len, max_batch_size
    )
    prompts = [
